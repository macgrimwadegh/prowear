...
function DropdownNavigationDark() {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (activeDropdown && !rootRef.current.contains(e.target as Node)) {
  ...
  const handleToggle = (itemName: string) => {
    if (itemName === "BASE WEAR" || itemName === "OUTER WEAR" || itemName === "WORK WEAR" || itemName === "HEAD WEAR") {
  ...
  const handleMouseEnter = (itemName: string) => {
    if (
      !isMobile &&
      (itemName === "BASE WEAR" || itemName === "OUTER WEAR" || itemName === "WORK WEAR" || itemName === "HEAD WEAR")
    ) {
      if (hoverTimeout) {
  ...
  const categories = [
    { name: "BASE WEAR", hasDropdown: true },
    { name: "OUTER WEAR", hasDropdown: true },
    { name: "WORK WEAR", hasDropdown: true },
    { name: "SWIM WEAR", hasDropdown: false },
    { name: "HEAD WEAR", hasDropdown: true },
  ]
  ...
  const headWearItems = [
    { name: "Caps", href: "/head-wear/caps" },
    { name: "Visors", href: "/head-wear/visors" },
  ]
  ...
  return (
    ...
    >
      {categories.map((category, index) => (
        <div key={category.name} className="flex items-center gap-2 sm:gap-4 shrink-0">
          <div
            className="relative"
          ...
          >
            {category.hasDropdown ? (
              <button
                ...
                onTouchStart={() => handleToggle(category.name)}
              ...
              <span className="hover:text-gray-300 transition-colors cursor-default">{category.name}</span>
            )}

            {/* Dropdown Menu for BASE WEAR */}
            {category.hasDropdown && activeDropdown === category.name && category.name === "BASE WEAR" && (
              <div
            ...
            {/* Dropdown Menu for OUTER WEAR */}
            {category.hasDropdown && activeDropdown === category.name && category.name === "OUTER WEAR" && (
              <div
                className="absolute top-full left-1/2 -translate-x-1/2 bg-white border border-gray-200 shadow-lg rounded-sm min-w-[180px] z-[9999] mt-1 max-w-[90vw]"
                onMouseEnter={() => handleMouseEnter(category.name)}
                onMouseLeave={handleMouseLeave}
            ...
            {/* Dropdown Menu for WORK WEAR */}
            {category.hasDropdown && activeDropdown === category.name && category.name === "WORK WEAR" && (
              <div
                className="absolute top-full left-1/2 -translate-x-1/2 bg-white border border-gray-200 shadow-lg rounded-sm min-w-[180px] z-[9999] mt-1 max-w-[90vw]"
                onMouseEnter={() => handleMouseEnter(category.name)}
                onMouseLeave={handleMouseLeave}
            ...
            {/* Dropdown Menu for HEAD WEAR */}
            {category.hasDropdown && activeDropdown === category.name && category.name === "HEAD WEAR" && (
              <div
                className="absolute top-full left-1/2 -translate-x-1/2 bg-white border border-gray-200 shadow-lg rounded-sm min-w-[180px] z-[9999] mt-1 max-w-[90vw]"
                onMouseEnter={() => handleMouseEnter(category.name)}
                onMouseLeave={handleMouseLeave}
          ...
          {/* Bullet separator (except for last item) */}
          {index < categories.length - 1 && <span className="mx-1 sm:mx-0">•</span>}
        </div>
      ))}
    </div>
  )
...
export default function HeavyTee5080Page() {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="flex justify-center bg-transparent text-transparent">
              <div className="w-full flex items-center justify-center p-4 bg-transparent">
                <img
                  src="/images/5080-heavy-tee-white.jpg"
                  alt="Heavy Tee 5080"
            ...
            <div className="space-y-8">
              {/* Product Title */}
              <div>
                <h1 className="text-3xl font-bold text-black uppercase tracking-wide mb-4">HEAVY TEE 5080</h1>
                <p className="text-gray-700 leading-relaxed">
                  Built to last with heavy weight 280 GSM 100% carded cotton.
                </p>
                <p className="text-gray-700 leading-relaxed mt-2">
                  A relaxed fit with dropped shoulders, wide neck ribbing, side seams, shoulder to shoulder tape, and
                  double needle hems ensure street-ready durability and comfort.
                </p>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-5 gap-3">
                  ...
                  <div className="w-8 h-8 rounded-full bg-[rgba(227,220,226,1)]"></div>
                  <div className="w-8 h-8 rounded-full bg-[rgba(245,240,216,1)]"></div>
                  <div className="w-8 h-8 rounded-full bg-[rgba(178,183,153,1)]"></div>
                  <div className="w-8 h-8 rounded-full bg-[rgba(147,142,113,1)]"></div>
                  <div className="w-8 h-8 rounded-full bg-[rgba(81,90,74,1)]"></div>
                  <div className="w-8 h-8 rounded-full bg-[rgba(29,52,42,1)]"></div>
                  <div className="w-8 h-8 rounded-full bg-[rgba(186,32,37,1)]"></div>
                  <div className="w-8 h-8 rounded-full bg-[rgba(107,90,74,1)]"></div>
                  <div className="w-8 h-8 rounded-full bg-[rgba(67,50,56,1)]"></div>
                  <div className="w-8 h-8 rounded-full bg-[rgba(29,32,44,1)]"></div>
                  <div className="w-8 h-8 rounded-full bg-[rgba(53,55,76,1)]"></div>
                  <div className="w-8 h-8 rounded-full bg-[rgba(228,224,212,1)]"></div>
                  <div className="w-8 h-8 rounded-full bg-[rgba(163,163,161,1)]"></div>
                </div>
              </div>
              ...
              <div className="pt-8 border-t border-gray-200">
                <p className="text-sm font-bold text-black uppercase tracking-wider">AS COLOUR HEAVY TEE 5080</p>
              </div>
