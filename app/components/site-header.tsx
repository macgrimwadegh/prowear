...
function DropdownNavigationDark() {
  ...
  const handleToggle = (itemName: string) => {
    if (itemName === "BASE WEAR" || itemName === "OUTER WEAR" || itemName === "WORK WEAR") {
      setActiveDropdown((prev) => (prev === itemName ? null : itemName))
    }
  }

  const handleMouseEnter = (itemName: string) => {
    if (!isMobile && (itemName === "BASE WEAR" || itemName === "OUTER WEAR" || itemName === "WORK WEAR")) {
      if (hoverTimeout) {
  ...
  const categories = [
    ...
    { name: "HEAD WEAR", hasDropdown: false },
  ]
  ...
  return (
    <div
      ref={rootRef}
      className="flex flex-nowrap items-center justify-center gap-2 sm:gap-4 text-[10px] sm:text-sm text-white uppercase tracking-wide overflow-x-auto md:overflow-visible whitespace-nowrap max-w-full pb-32 sm:pb-36 md:pb-40 mx-auto"
    >
        ...
        <div key={category.name} className="flex items-center gap-2 sm:gap-4 shrink-0">
          ...
          >
            ...
            {category.hasDropdown && activeDropdown === category.name && category.name === "BASE WEAR" && (
              <div
                className="absolute top-full left-1/2 -translate-x-1/2 bg-white border border-gray-200 shadow-lg rounded-sm min-w-[180px] z-[9999] mt-2 max-w-[90vw]"
                onMouseEnter={() => handleMouseEnter(category.name)}
                onMouseLeave={handleMouseLeave}
              >
                <ul className="py-1">
...
export default function SiteHeader() {
  return (
    <header className="w-full py-4 sm:py-6 bg-black">
      <nav className="flex flex-col items-center justify-center w-full space-y-3 sm:space-y-4">
        {/* Top - PROWEAR Logo */}
        <Link href="/" className="flex-shrink-0 mx-auto">
          ...
          >
            ...
            <path d="M399.82,45.61c3.91-4.38,7.48-8.36,11.05-12.34,7.2-8.03,14.46-15.99,21.57-24.1,4.49-5.13,9.9-6.92,16.34-4.78,1.9.63,3.81,1.9,5.15,3.39,10.04,11.26,19.94,22.65,29.89,34,1.02,1.16,2.01,2.34,3.21,3.73-.68.12-1.03.22-1.39.23-5.49,0-10.98.05-16.48-.05-.84-.01-2.03-.49-2.45-1.15-2.69-4.19-6.4-5.28-11.28-5.04-8.09.42-16.21.12-24.31.11-.76,0-1.52-.06-2.75-.12,3.35-3.64,6.35-6.97,9.45-10.21.36-.38,1.20-.42,1.81-.43,3.53-.04,7.06-.02,10.60-.02.62,0,1.24-.07,2.28-.14-3.01-3.52-5.75-6.74-8.69-10.19-.58.54-1.16.98-1.64,1.52-7.2,8.03-14.4,16.06-21.55,24.12-.99,1.10-1.97,1.67-3.51,1.64-5.04-.10-10.07-.04-15.10-.05-.57,0-1.13-.07-2.21-.14" />
            <path d="M2.97,14.52c3.34-3.28,6.66-6.57,10.04-9.79.4-.38,1.20-.43,1.82-.43,11.25-.02,22.49-.08,33.73,0,5.88.05,11.60,1.05,16.75,4.13,9.18,5.48,9.36,16.76.25,22.35-2.30,1.41-4.95,2.43-7.58,3.04-3.28.76-6.70,1.19-10.08,1.25-8.95.16-17.91.1-26.86.01-1.66-.01-2.16.52-2.09,2.13.11,2.72.03,5.45.03,8.31H2.84c-.04-.80-.10-1.51-.10-2.20,0-5.43,0-10.85,0-16.28,0-2.59,0-2.60,2.52-2.60,14.19,0,28.38.05,42.56-.06,1.90-.02,3.84-.61,5.66-1.23,1.54-.54,2.56-1.98,2.14-3.56-.33-1.24-1.26-2.78-2.33-3.26-1.78-.79-3.90-1.12-5.88-1.14-13.98-.08-27.97-.05-41.97-.04h-2.13c-.12-.21-.22-.42-.34-.63" />
          </svg>
        </Link>
